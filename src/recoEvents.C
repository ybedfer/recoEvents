...
#define FromChain
  ...
  */
  ...
  for (Long64_t jentry=firstEvent; jentry<nentries+firstEvent;jentry++) {
	...
	// -> "coalescedHs"
	const SimTrackerHitData &hit = hits[idet]->at(ih);
	...
	updateDetEvent(idet,ih);
      ...
      for (int ih = 0; ih<(int)coalescedHs.size(); ih++) {
	// ********** LOOP ON coalescedHs
	debugHit(idet,ih);
	initStatusWord(ih);
	// ***** MODULE SELECTION
	if (!moduleRequirementOK(idet,ih)) continue;
	// ***** HIT SELECTION
	if (!PDGRequirementOK(idet,ih)) continue;
	if (!primaryRequirementOK(ih)) continue;
	if (!borderRequirementOK(idet,ih)) continue;
	if (!edgeRequirementOK(idet,ih)) continue;
	setStatusWord(ih);
	// ***** FILL sim HISTO
	// - It's "coa" rather than "sim"
	fillSimHit(idet,ih);
	debugHit(idet,ih);
      }
	...
	if (im==Rec2coas.end()) {
	  ...
	  int is, selecRec; for (is=selecRec = 0; is<(int)coas.size(); is++) {
	    ...
	    if (cIndex<0 || nHits<=cIndex) {
	      ...
	      debugHitRec(idet,is,(int)coas.size(),cIndex,iR);
	      // ***** SELECTIONS
	...
	continue;
      for (int ir = 0; ir<(int)raws[idet]->size(); ir++) {
	// ********** LOOP ON raw HITS
	// ***** REFERENCE TO coa HIT
	map<int,vector<int>,less<int>>::const_iterator im = raw2coas.find(ir);
	if (im==raw2coas.end()) {
	  printf("#%5d Warning: det %d: raw %d not associated\n",
		 evtNum,idet,ir);
	}
	else {
	  const vector<int> &coas = im->second;
...
unsigned int recoEvents::getSimHitStatus(int idet, int ih)
{
...
void recoEvents::initStatusWord(int ih)
{
  // -> 8-bit status word stored in SimTrackerHitData::quality, taking
  // advantage of the fact that only bits 30,31 have a predetermined meaning.
  //  0x01: "setStatusWord" has been applied.
  //  0x02: All requirements fulfilled
  //  0x04: PDG requirement fulfilled
  //  0x08: quality>0 (as opposed to quality>1) requirement fulfilled
  //  0x10: quality>1 requirement fulfilled
  //  0x20: Border requirement fulfilled
  //  0x40: Edge requirement fulfilled
  SimTrackerHitData &hit = coalescedHs[ih]; // Not const
  // Quality
  std::int32_t &quality = hit.quality; // From original SimHit, to be updated
  quality |= 0x1;
}
bool recoEvents::PDGRequirementOK(int idet, int ih)
{
  if (!requirePDG) return true;
  SimTrackerHitData &hit = coalescedHs[ih]; // Not const
  // "ih" is index into array of "coalescedHs".
  // => Let's get index into array of SimHits
  for (auto is = sim2coa.cbegin(); is!=sim2coa.cend(); is++) {
    if (is->second==ih) {
      int jh = is->first;
      MCParticleData &part = getMCParticle(idet,jh);
      return part.PDG==requirePDG;
    }
  }
  printf("%5d: coalescedHs[%d] is not mapped to any SimHit\n",evtNum,ih);
  return false;
}
bool recoEvents::primaryRequirementOK(int ih) {
  if (!requireQuality) return true;
  SimTrackerHitData &hit = coalescedHs[ih]; // Not const
  std::int32_t quality = hit.quality;
  const int BITProducedBySecondary = 30;
  return !(quality & 1 << BITProducedBySecondary);
}
void recoEvents::setStatusWord(int ih)
{
  SimTrackerHitData &hit = coalescedHs[ih]; // Not const
  std::int32_t &quality = hit.quality;
  /* All requirements fulfilled */ quality |= 0x02;
  if (requirePDG)                  quality |= 0x04;
  if (requireQuality)              quality |= 0x08;
  if (requireQuality>1)            quality |= 0x10;
  if (requireOffBorder)            quality |= 0x20;
  if (requireOffEdge)              quality |= 0x40;
}
unsigned short recoEvents::getStatusWord(int ih)
{
  // Returned status: see "setStatusWord".
...
bool recoEvents::setXReferences(int idet)
{
  // Init
  raw2Rec.clear(); Rec2coas.clear(); raw2coas.clear();
  // Checks: size of association arrays match
  ...
  for (int ih = 0; ih<nArhs; ih++) {
	...
	int is, match; for (is=match = 0; is<(int)coas.size(); is++) {
    ...
    // raw -> coa
    map<int,vector<int>>::iterator im = raw2coas.find(rIndex);
    if (im==raw2coas.end()) {
      vector<int> coas; coas.push_back(cIndex); raw2coas[rIndex] = coas;
    } else {
      vector<int> &coas = im->second;
      // Not twice same SimHit associated to rawHit.
      int is, match; for (is=match = 0; is<(int)coas.size(); is++) {
	if (coas[is]==cIndex) { match = 1; break; }
      }
      if (!match) coas.push_back(cIndex);
...
void recoEvents::fillSimHit(int idet, int ih)
{
  const SimTrackerHitData &hit = coalescedHs[ih];
...
void recoEvents::fillRecHit(int idet, int ir)
{
			 ...
			 unsigned long cellID)
{
  ...
  if      (idet==0) { // Special CyMBaL: fill reduced Radius
    ...
    hs->phir->Fill(phil,div);
    hs->Rr->Fill(Rr,div);
    // Rphi: let's used fixed radius (set from geometry) instead hit radius, so
    // as to keep Rphi range w/in detector width.
    hs->xyr->Fill(Zl,getCyMBaLRadius(cellID)*phil);
  }
  else if (idet==1) { // Special Outer: fill Rcosphi, Ul, Vl
    double Vl = (Yl+Xl)/sqrt(2), Ul = (Yl-Xl)/sqrt(2), Rcdphi = Zl+radii[1][0];
    hs->Rr->Fill(Rcdphi,div);
    hs->Ur->Fill(Ul,div); hs->Vr->Fill(Vl,div);
    ...
    if (simOrRec==0 && (fabs(Xl)>hWidths[1][0] || fabs(Yl)>ZHLengths[1]))
      printf("=== %5d 0x%08lx,0x%08lx %.4f,%.4f,%.4f %.4f,%.4f,%.4f\n",
	     evtNum,cellID&0xffffffff,cellID>>32,X,Y,Z,Xl,Yl,Zl);
#endif
    hs->xyr->Fill(Yl,Xl);
  }
}
bool recoEvents::fillResids(int idet, int ih, int ir)
{
  ...
  const Vector3f &pos = rec.position;
  unsigned long cellID = rec.cellID;
  // Debugging
  ...
  if      (idet==0) { // CyMBaL specific
    double RrR = sqrt(XR*XR+YR*YR), phiR = atan2(YR,XR);
    double RrS = sqrt(XS*XS+YS*YS), phiS = atan2(YS,XS);
    double dRr = 1000*(RrR-RrS), dphir = 1000*(phiR-phiS);
    rs.R->Fill(dRr);
    requestDebug(idet,0x10000);
    if (fabs(dRr)>200 && debugIsOn(0x10000)) {
      printf("%5d 0x%08lx,0x%08lx dRr = %4.0f µm\n",
	     evtNum,cellID&0xffffffff,cellID>>32,dRr);
    }
    double Rdphi = getCyMBaLRadius(cellID)*dphir; rs.Rphir->Fill(Rdphi);
    ...
    double VR = (YR+XR)/sqrt(2), UR = (YR-XR)/sqrt(2), RcdphiR = ZR+radii[1][0];
    double VS = (YS+XS)/sqrt(2), US = (YS-XS)/sqrt(2), RcdphiS = ZS+radii[1][0];;
    double dRcdphi = 1000*(RcdphiR-RcdphiS); rs.R->Fill(dRcdphi);
    requestDebug(idet,0x10000);
    if (fabs(dRcdphi)>200 && debugIsOn(0x10000)) {
      printf("%5d 0x%08lx,0x%08lx dRr = %4.0f µm\n",
	     evtNum,cellID&0xffffffff,cellID>>32,dRcdphi);
    }
...
bool recoEvents::borderRequirementOK(int idet, int ih)
{
  ...
  const Vector3d &pos = hit.position;
  double X = pos.x, Y = pos.y, Z = pos.z;
  unsigned int onBorder = isOnBorder(idet,hit.cellID,X,Y,Z);
  if (requireOffBorder>0) return !onBorder;
				    ...
				    double Xs, double Ys, double Zs)
{
  ...
  double Xl, Yl, Zl; if (idet<2) wTol(idet,cellID,Xs,Ys,Zs,Xl,Yl,Zl);
  ...
  if (idet==0) {
    ...
    double Vl = (Yl+Xl)/sqrt(2), Ul = (Yl-Xl)/sqrt(2);
    for (int strip = 0; strip<2; strip++) {
      double W = strip==0 ? Ul : Vl;
...
bool recoEvents::edgeRequirementOK(int idet, int ih)
{
  if (!requireOffEdge) return true;
  const SimTrackerHitData &hit = coalescedHs[ih];
  bool cross = crossEdge(idet,hit);
  if (requireOffEdge>0) return !cross;
  else                  return cross;
}
bool recoEvents::crossEdge(int idet, const SimTrackerHitData &hit)
{
  bool cross = false;
  ...
  double lpos[3], &Xl = lpos[0], &Yl = lpos[1], &Zl = lpos[2];
  double dummy[3];
  // Global -> Local
  double lmom[3]; if (idet<2) {
    wTol(idet,cID,Mx,My,Mz,Px,Py,Pz,Xl,Yl,Zl,lmom);
  }
  if (idet==0) {
    // Volume parameters
    ...
    for (int io = 0; io<2; io++) {
      double rT = radii[0][staveType]+(2*io-1)*volumeThicknesses[0]/2;
      unsigned int status = cExtension(lpos,lmom,rT,dZ,startPhi,endPhi);
      cross |= status!=0;
    }
  }
  else if (idet==1) {
    ...
    // Loop on extending to inner/outer volume walls
    for (int io = 0; io<2; io++) {
      double zT = (2*io-1)*volumeThicknesses[1]/2;
      unsigned int status = bExtension(lpos,lmom,zT,dX,dY);
      cross |= status!=0;
    }
  }
  return cross;
}
void recoEvents::fillRawHit(int idet, int ir)
{
  edm4eic::RawTrackerHitData &raw = raws[idet]->at(ir);
  unsigned long cellID = raw.cellID;
  unsigned int module, div, strip; parseCellID(idet,cellID,module,div,strip);
  // ***** STRIP: Convert stripID -> strip. Is it valid? 
...
bool recoEvents::extrapolate(int idet, int ih, int jh)
{
  ...
  if (!doPrint && !ok) {
    // Do print debugging message, even if not requested, when extrapolation
...
bool recoEvents::coalesce(int idet, vector<int> coalesced, SimTrackerHitData &hext) {
  ...
  int ihs[5] = {-1,-1,-1,-1,-1};
  int ic; unsigned int hPat; double eDep, pathLength;
  for (ic = 0, eDep=pathLength = 0, hPat = 0; ic<(int)coalesced.size(); ic++) {
    ...
    eDep += hit.eDep; pathLength += hit.pathLength;
  }
  if ((hPat&0x18)==0x18) { // ***** COALESCE SUBVOLUMES 0x3 anD 0x4
    ...
    hext.position = pext; hext.eDep = eDep; hext.pathLength = pathLength;
    ...
    SimTrackerHitData &hit = hits[idet]->at(ih);
    extend(idet,ih,hext);
    hext.eDep = eDep; hext.pathLength = pathLength; hext.quality = hit.quality;
    hext.cellID = hit.cellID;
    if (doDebug) {
    ...
    SimTrackerHitData &hit = hits[idet]->at(ih);
    hext.position = hit.position; hext.eDep = eDep; hext.pathLength = pathLength;
  ...
  if (doDebug) {
    ...
    double Mx = pos.x, My = pos.y, Mz = pos.z;
    double Xl, Yl, Zl; wTol(idet,cID,Mx,My,Mz,Xl,Yl,Zl);
    if (idet==0) {
...
void recoEvents::extend(int idet, int ih, SimTrackerHitData &hext)
{
  ...
  hext.cellID = cID; hext.quality = hit.quality;
  if (debugIsOn(0x100)) {
    printf("hit %2d/%-2d: extend? %d\n",ih,(int)hits[idet]->size(),doExtend);
    double Nx, Ny, Nz; wTol(idet,cID,Mx,My,Mz,Nx,Ny,Nz);
    double Fx, Fy, Fz; wTol(idet,cID,Ex,Ey,Ez,Fx,Fy,Fz);
    if (idet==0) {
      double R0 = getCyMBaLRadius(cID);
      double NR = sqrt(Nx*Nx+Ny*Ny), FR = sqrt(Fx*Fx+Fy*Fy);
      printf(" 0x%08lx,0x%08lx %.2f,%.2f(%.3f),%.2f",
	     cID&0xffffffff,cID>>32,Nx,Ny,NR-R0,Nz);
      if (doExtend)
	printf(" -> %.2f,%.2f(%.3f),%.2f",Fx,Fy,FR-R0,Fz);
      printf(" mm\n");
    }
    else if (idet==1) {
      printf(" 0x%08lx 0x%08lx %.2f,%.2f,%.2f (%6.3f)",
	     cID&0xffffffff,cID>>32,Mx,My,Mz,Nz);
      if (doExtend)
	printf(" -> %.2f,%.2f,%.2f (%6.3f)",Ex,Ey,Ez,Fz);
      printf(" mm\n");
    }
  }
}
				 ...
				 double &pathDepth, double &depth)
{
  ...
  double Xl, Yl, Zl, lmom[3]; if (idet<2 && (subVolID==0x3 || subVolID==0x4)) {
    wTol(idet,cID,Mx,My,Mz,Px,Py,Pz,Xl,Yl,Zl,lmom);
  }
  if (idet==0  && (subVolID==0x3 || subVolID==0x4)) {
    double Rr = sqrt(Xl*Xl+Yl*Yl);
    double Pxr = lmom[0], Pyr = lmom[1];
    double cTheta = (Pxr*Xl+Pyr*Yl)/P/sqrt(Xl*Xl+Yl*Yl);
    ...
    double Pxr = lmom[2];
    double cTheta = Pxr*abs(Zl)/P/sqrt(Zl*Zl);
    pathDepth = hit.pathLength*cTheta; depth = Zl;
    double thickness = radiatorThicknesses[idet];
    traversing = fabs(fabs(pathDepth)-thickness)<tolerance;
  }
  return traversing;
...
{
  // Coalesced hits
  coalescedHs.clear(); sim2coa.clear();
  // LayerModules
  prvLayerModule.init();
...
bool recoEvents::debugIsBooked(unsigned int level)
{
...
void recoEvents::updateDetEvent(int idet, int ih)
{
  ...
  // LONE PRIMARY?
  const int BITProducedBySecondary = 30;
  if (!(hit.quality&1<<BITProducedBySecondary)) { // Primary
...
bool recoEvents::moduleRequirementOK(int idet, int ih)
{
  const SimTrackerHitData &hit = coalescedHs[ih];
...
void recoEvents::debugHit(int idet, int ih)
{
  ...
  if (debugIsOn(0x1)) {
    ...
    const SimTrackerHitData &hit = coalescedHs[ih];
    const Vector3d &pos = hit.position;
    unsigned short status = getStatusWord(ih);
    printf("hit %2d/%-2d: 0x%08lx,0x%08lx X,Y,Z %7.2f,%7.2f,%8.2f status 0x%x\n",
	   ih,nCoaHs,hit.cellID&0xffffffff,hit.cellID>>32,
...
void recoEvents::debugHitRec(int idet, int is, int ncoas, int cIndex, int ir)
{
  if ((verbose&0x10<<idet) || evtNum==evtToDebug) {
    const SimTrackerHitData &hit = coalescedHs[cIndex];
    int nRecs = recs[idet]->size();
    edm4eic::TrackerHitData &rec = recs[idet]->at(ir);
...
void recoEvents::debugAssoc(int idet)
{
  ...
  if (debugIsOn(0x10)) {
    ...
    for (int ih = 0; ih<nArhs; ih++) {
      ...
      int rIndex = arh.index;
      edm4eic::RawTrackerHitData &raw = raws[idet]->at(rIndex);
      printf(" %d: %d(0x%08lx)",ih,rIndex,raw.cellID&0xffffffff);
    }
    printf("\nashs %d:",nAshs);
    for (int ih = 0; ih<nAshs; ih++) {
      podio::ObjectID &ash = ashs[idet]->at(ih);
      int sIndex = ash.index;
      SimTrackerHitData &sim = hits[idet]->at(sIndex);
      printf(" %d: %d(0x%08lx),",ih,sIndex,sim.cellID&0xffffffff);
    }
    printf("\naRhs %d:",nARhs);
    for (int iR = 0; iR<nARhs; iR++) {
      podio::ObjectID &aRh = aRhs[idet]->at(iR);
      printf(" %d: %d,",iR,aRh.index);
    }
    printf("\n");
  }
}
void recoEvents::debugMaps(int idet)
{
  ...
  for (im = raw2coas.cbegin(); im != raw2coas.cend(); im++) {
    printf("raw2coas: %d ->",im->first);
    const vector<int> &coas = im->second;
    for (int is = 0; is<(int)coas.size(); is++) printf(" %d",coas[is]);
    printf("\n");
  }
  ...
  if (idet==0) {
    ...
    if (debugIsOn(0x100)) {
      ...
      printf(" extendHit(%d,%d(0x%08lx,0x%08lx)) %.4f-(%d)->%.4f: 0x%x\n",
	     idet,ih,cID&0xffffffff,cID>>32,Rr-R0,pm*direction,rT-R0,status);
      printf("  %.2f,%.2f(%.2f),%.2f mm %.4f,%.4f,%.4f GeV",
	     Xl,Yl,Rr-R0,Zl,lmom[0],lmom[1],lmom[2]);
      if (status&0xf) {
	double Ex = lend[0], Ey = lend[1], Ez = lend[2], ER = sqrt(Ex*Ex+Ey*Ey);
	printf(" -> %.2f,%.2f(%.4f),%.2f mm\n",Ex,Ey,ER-R0,Ez);
      }
      else
	printf("\n");
    }
    if (!(status&0xf)) return false;
    else {
      double gend[3]; lTow(0,cID,lend,gend);
    ...
    if (!(status&0x1)) {
      printf("#%5d: extendHit(%d,%d(0x%08lx,0x%08lx)) %.4f-(%d)->%.4f inconsistency: 0x%x\n",
	     evtNum,idet,ih,cID&0xffffffff,cID>>32,Zl,pm*direction,zT,status);
      printf("%.2f,%.2f,%.2f mm %.4f,%.4f,%.4f GeV\n",
	     Xl,Yl,Zl,lmom[0],lmom[1],lmom[2]);
      return false;
    }    
    else {
      double gend[3]; lTow(1,cID,lend,gend);
      for (int i = 0; i<3; i++) gext[i] = (gini[i]+gend[i])/2;
    }
    /*
			    ...
			    bool addToPreExisting)
{
  ...
  for (int idet = 0; idet<N_DETs; idet++) {
    ...
    const Vector3f &mom = hit.momentum; double Px = mom.x, Py = mom.y, Pz = mom.z;
    // Locals
    double Xl, Yl, Zl, lmom[3]; wTol(idet,cID,Mx,My,Mz,Px,Py,Pz,Xl,Yl,Zl,lmom);
    double lpos[3]; lpos[0] = Xl/10; lpos[1] = Yl/10; lpos[2] = Zl/10;
    if (idet==0) {
      // Pathlength
	...
	rMin,rMax,startPhi,endPhi,dZ);
      ...
      // Globals -> Locals
      double lzxy[3]; lzxy[0] = lmom[2]; lzxy[1] = lmom[0]; lzxy[2] = lmom[1];
      // Pathlength
      double path = hit.pathLength/10;
      // Parameters
	...
	rMin,rMax,startX,endX,dZ);
      ...
      // DrawHit
      if (addToPreExisting) AddHit(       lpos,lzxy,path);
      else                  bDrawHit(pars,lpos,lzxy,path,r2s);
    }
  }
}
#include "TEllipse.h"
